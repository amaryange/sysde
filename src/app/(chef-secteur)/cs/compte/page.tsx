          ...
          <Card className='h-100'>
            <CardBody>
              ...
              <Form onSubmit={handleSaveInfo}>
                <FormGroup>
                  <Label for='cs-name'>Nom affiché</Label>
                  <Input id='cs-name' type='text' value={name} onChange={(e) => setName(e.target.value)} placeholder='Votre nom' />
                </FormGroup>
                <FormGroup>
                  <Label for='cs-email'>Adresse e-mail</Label>
                  <Input id='cs-email' type='email' value={email} onChange={(e) => setEmail(e.target.value)} placeholder='votre@email.com' />
                </FormGroup>
                <FormGroup>
                  <Label>Rôle</Label>
                  <Input type='text' value='Chef Secteur' disabled style={{ opacity: 0.6, cursor: 'not-allowed' }} />
                </FormGroup>
          ...
          <Card className='h-100'>
            <CardBody>
              ...
              <Form onSubmit={handleChangePassword}>
                <FormGroup>
                  <Label for='cs-pwd-current'>Mot de passe actuel</Label>
                  <PasswordInput id='cs-pwd-current' value={currentPwd} onChange={setCurrentPwd} placeholder='••••••••' />
                </FormGroup>
                <FormGroup>
                  <Label for='cs-pwd-new'>Nouveau mot de passe</Label>
                  <PasswordInput id='cs-pwd-new' value={newPwd} onChange={setNewPwd} placeholder='••••••••' />
                  <small className='text-muted'>Minimum 6 caractères.</small>
                </FormGroup>
                <FormGroup>
                  <Label for='cs-pwd-confirm'>Confirmer le mot de passe</Label>
                  <PasswordInput id='cs-pwd-confirm' value={confirmPwd} onChange={setConfirmPwd} placeholder='••••••••' />
                </FormGroup>
