...
// ---------------------------------------------------------------------------
// Types — postes encadreurs hors CS
// ---------------------------------------------------------------------------
type Collaborateur = {
  id:                   string;
  lib:                  string;
...
// ---------------------------------------------------------------------------
// Options — rôles encadreurs hors CS
// ---------------------------------------------------------------------------
const ROLES_OPT: ComboboxOption[] = MOCK_ROLES
  .filter((r) => r.encadreur && r.acronyme !== 'CS')
  .map((r) => ({ value: r.id, label: `${r.acronyme} — ${r.nom}` }));

const ROLES_FILTER: ComboboxOption[] = [
  { value: '', label: 'Tous' },
  ...MOCK_ROLES.filter((r) => r.encadreur && r.acronyme !== 'CS').map((r) => ({ value: r.acronyme, label: r.acronyme })),
];
...
const INITIAL: Collaborateur[] = [
  { id: 'co-1', lib: 'Contrôleur Formation Abengourou', cde: 'CF-ABG-001', nbre_postes: 2, role: 'rl-2', id_operateur_secteur: 'os-1', cde_secteur: '001', lib_secteur: 'Abengourou',   cde_lot: 'LT-01',       num_lot: '1',    cde_section: 'A',    region: 'Indénié-Djuablin', departement: 'Abengourou',   sprefecture: 'Abengourou',   actif: true  },
  { id: 'co-2', lib: 'Contrôleur Ordinaire Abengourou', cde: 'CO-ABG-001', nbre_postes: 3, role: 'rl-3', id_operateur_secteur: 'os-1', cde_secteur: '001', lib_secteur: 'Abengourou',   cde_lot: 'LT-02',       num_lot: '2',    cde_section: 'A',    region: 'Indénié-Djuablin', departement: 'Abengourou',   sprefecture: 'Abengourou',   actif: true  },
  { id: 'co-3', lib: 'Formateur Saigné Abengourou',     cde: 'FS-ABG-001', nbre_postes: 8, role: 'rl-4', id_operateur_secteur: 'os-1', cde_secteur: '001', lib_secteur: 'Abengourou',   cde_lot: 'LT-01,LT-02', num_lot: '1, 2', cde_section: 'A, B', region: 'Indénié-Djuablin', departement: 'Abengourou',   sprefecture: 'Abengourou',   actif: true  },
  { id: 'co-4', lib: 'Moniteur Bondoukou 1',            cde: 'MO-BDK-001', nbre_postes: 3, role: 'rl-5', id_operateur_secteur: 'os-3', cde_secteur: '002', lib_secteur: 'Bondoukou',    cde_lot: 'LT-04',       num_lot: '4',    cde_section: 'C',    region: 'Zanzan',           departement: 'Bondoukou',    sprefecture: 'Bondoukou',    actif: true  },
  { id: 'co-5', lib: 'Équipe Spéciale Daoukro',         cde: 'ES-DKR-001', nbre_postes: 1, role: 'rl-6', id_operateur_secteur: 'os-4', cde_secteur: '005', lib_secteur: 'Daoukro',      cde_lot: 'LT-10',       num_lot: '10',   cde_section: 'E',    region: "N'Zi-Comoé",      departement: 'Daoukro',      sprefecture: 'Daoukro',      actif: true  },
  { id: 'co-6', lib: 'Formateur Saigné Tanda',          cde: 'FS-TDA-001', nbre_postes: 4, role: 'rl-4', id_operateur_secteur: 'os-5', cde_secteur: '004', lib_secteur: 'Tanda',        cde_lot: 'LT-08,LT-09', num_lot: '8, 9', cde_section: 'D',    region: 'Zanzan',           departement: 'Tanda',        sprefecture: 'Tanda',        actif: false },
];
...
const ROLE_BADGE_COLORS: Record<string, string> = {
  CF: 'warning', CO: 'info', FS: 'success', MO: 'primary', ES: 'secondary', SE: 'dark',
};

const emptyForm = (): CollaborateurForm => ({
...
const fillForm = (c: Collaborateur): CollaborateurForm => {
  const os    = MOCK_OPERATEUR_SECTEUR.find((o) => o.id === c.id_operateur_secteur);
  ...
  const osOpt = opOpt
    ? MOCK_OPERATEUR_SECTEUR
        ...
        .find((o) => o.value === c.id_operateur_secteur) ?? null
    : null;
  const lotsDisponibles = MOCK_OPERATEUR_LOT
    .filter((ol) => ol.id_operateur_secteur === c.id_operateur_secteur)
    .map((ol) => ({ value: ol.cde_lot, label: `Lot ${ol.num_lot} — ${ol.cde_lot}` }));
  const selectedLots = c.cde_lot
    ? c.cde_lot.split(',').map((v) => lotsDisponibles.find((l) => l.value === v.trim())).filter(Boolean) as ComboboxOption[]
    : [];
  return {
    lib: c.lib, cde: c.cde, nbre_postes: c.nbre_postes,
    role: ROLES_OPT.find((r) => r.value === c.role) ?? ROLES_OPT[0],
    operateur: opOpt, operateur_secteur: osOpt,
    cde_secteur: c.cde_secteur, lib_secteur: c.lib_secteur,
    lots: selectedLots, cde_section: c.cde_section,
    region: c.region, departement: c.departement, sprefecture: c.sprefecture,
    actif: c.actif,
  };
};
...
const CollaborateurList = () => {
  ...
  const [form,         setForm        ] = useState<CollaborateurForm>(emptyForm);

  const [fCde,    setFCde   ] = useState(() => searchParams.get('col_cde')  ?? '');
  const [fLib,    setFLib   ] = useState(() => searchParams.get('col_lib')  ?? '');
  const [fRole,   setFRole  ] = useState(() => searchParams.get('col_role') ?? '');
  const [fOp,     setFOp    ] = useState(() => searchParams.get('col_op')   ?? '');
  const [fSec,    setFSec   ] = useState(() => searchParams.get('col_sec')  ?? '');
  const [fStatut, setFStatut] = useState(() => searchParams.get('col_sta')  ?? '');
  const [page,    setPage   ] = useState(() => Number(searchParams.get('col_page') ?? '1'));
  ...
  const debCde = useDebouncedCallback((v: string) => pushUrl({ col_cde: v || null, col_page: null }), 300);
  const debLib = useDebouncedCallback((v: string) => pushUrl({ col_lib: v || null, col_page: null }), 300);
  const debSec = useDebouncedCallback((v: string) => pushUrl({ col_sec: v || null, col_page: null }), 300);
  ...
  const handleRole   = (v: string) => { setFRole(v);   setPage(1); pushUrl({ col_role: v || null, col_page: null }); };
  const handleOp     = (v: string) => { setFOp(v);     setPage(1); pushUrl({ col_op:   v || null, col_page: null }); };
  const handleStatut = (v: string) => { setFStatut(v); setPage(1); pushUrl({ col_sta:  v || null, col_page: null }); };
  const handlePage   = (p: number) => { setPage(p); pushUrl({ col_page: p > 1 ? String(p) : null }); };
  ...
  const handleDeleteConfirm = () => {
    ...
    log('DELETE', 'Collaborateur', `Suppression du collaborateur ${deleteTarget.cde}`, deleteTarget.cde);
  ...
  const handleSave = () => {
    ...
    if (editing) {
      setData((prev) => prev.map((c) => c.id === editing.id ? { id: editing.id, ...payload } : c));
      log('UPDATE', 'Collaborateur', `Modification du collaborateur ${form.cde}`, form.cde);
    } else {
      setData((prev) => [...prev, { id: `col-${Date.now()}`, ...payload }]);
      log('CREATE', 'Collaborateur', `Création du collaborateur ${form.cde}`, form.cde);
    }
    setModal(false);
  };
  ...
  const filtered = useMemo(() => {
    const base = isLocked
      ? data.filter((c) => MOCK_OPERATEUR_SECTEUR.find((os) => os.id === c.id_operateur_secteur)?.id_operateur === userOpId)
      : data;
    return base.filter((c) => {
      const acr = roleAcronyme(c.role);
      return (
        (!fCde    || c.cde.toLowerCase().includes(fCde.toLowerCase())) &&
        (!fLib    || c.lib.toLowerCase().includes(fLib.toLowerCase())) &&
        (!fRole   || acr === fRole) &&
        (!fOp     || MOCK_OPERATEUR_SECTEUR.find((o) => o.id === c.id_operateur_secteur)?.id_operateur === fOp) &&
        (!fSec    || c.lib_secteur.toLowerCase().includes(fSec.toLowerCase()) || c.cde_secteur.includes(fSec)) &&
        (!fStatut || (fStatut === 'Actif' ? c.actif : !c.actif))
      );
    });
  }, [data, fCde, fLib, fRole, fOp, fSec, fStatut, isLocked, userOpId]);
  ...
  return (
    <div>
            ...
            <Table className='table table-hover mb-0' style={{ minWidth: 620 }}>
              ...
              <tbody>
                {paginated.length === 0 ? (
                  ...
                  return (
                    <tr key={c.id}>
                      <td><code>{c.cde}</code></td>
                      <td className='f-w-600' style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.lib}</td>
                      <td className='text-center'>{c.nbre_postes}</td>
                      <td><Badge color={ROLE_BADGE_COLORS[acr] ?? 'secondary'} className='badge-light'>{acr}</Badge></td>
                      <td>{operateurFromOs(c.id_operateur_secteur)}</td>
                      <td className='text-muted'>
                        <span className='me-1' style={{ fontSize: 11, color: '#aaa' }}>{c.cde_secteur}</span>
                        {c.lib_secteur}
                      </td>
                      <td><Badge color={c.actif ? 'success' : 'secondary'} className='badge-light'>{c.actif ? 'Actif' : 'Inactif'}</Badge></td>
                      <td><RowActions prefix='col' id={c.id} onView={() => openView(c)} onEdit={() => openEdit(c)} onDelete={() => setDeleteTarget(c)} /></td>
                    </tr>
                  );
                })}
              </tbody>
      ...
      <AppDrawer
        ...
        title={viewing ? 'Détails — Collaborateur' : editing ? 'Modifier le collaborateur' : 'Ajouter un collaborateur'}
        ...
        <Form onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
          ...
          <Row>
            <Col xs='12' sm='8'>
              <FormGroup>
                <Label>Libellé</Label>
                <Input disabled={viewing} value={form.lib} onChange={(e) => setForm((f) => ({ ...f, lib: e.target.value }))} placeholder='Ex: Contrôleur Formation Abengourou' />
              </FormGroup>
            </Col>
            <Col xs='12' sm='4'>
              <FormGroup>
                <Label>Code <span className='text-danger'>*</span></Label>
                <Input disabled={viewing} value={form.cde} onChange={(e) => setForm((f) => ({ ...f, cde: e.target.value }))} placeholder='CF-ABG-001' />
              </FormGroup>
            </Col>
          </Row>
          <Row>
      ...
      <ConfirmDelete
        isOpen={!!deleteTarget}
        message={`Voulez-vous vraiment supprimer le collaborateur "${deleteTarget?.cde}" ? Cette action est irréversible.`}
